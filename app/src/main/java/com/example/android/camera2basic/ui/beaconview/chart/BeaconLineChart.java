...
public class BeaconLineChart extends BeaconChart {
    ...
    protected float processReturnValue(Beacon beacon, AdvertisingPacket advertisingPacket, float rssi) {
        switch (valueType) {
            case VALUE_TYPE_RSSI: {
                Log.d("beacon","NON filtered rssi="+advertisingPacket.getRssi());
                return advertisingPacket.getRssi();
            }
            case VALUE_TYPE_RSSI_FILTERED: {
               // Log.d("beacon","Filtered rssi="+rssi);
                return rssi;
            }
            case VALUE_TYPE_DISTANCE: {
                //if (beacon.getMacAddress().equalsIgnoreCase("F1:C1:35:3A:CE:CB"))
                   // Log.d("beacon","Distance in cm = "+(100*BeaconDistanceCalculator.calculateDistanceTo(beacon, rssi)));
                float distance= BeaconDistanceCalculator.calculateDistanceTo(beacon, rssi);
                Log.d("beacon","1-distance to "+beacon.getName()+" is ="+distance);
                Log.d("beacon","2-distance to "+beacon.getName()+" from beacon.getDistance() is ="+beacon.getDistance());
                return distance;
            }
            case VALUE_TYPE_FREQUENCY: {
