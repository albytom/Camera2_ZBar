...
void
getPathThroughMaze(unsigned char *outputImage, unsigned char *binaryImage, int w, int h, int startX,
                   int startY, int endX, int endY, bool drawCostMap,
                   Mat floor_img) {
    ...
    while (iter_count <= 3) {
            ...
            for (int i = temp1; i < 2; i++) {
                for (int j = temp2; j < 2; j++) {
                    if (currX + j < 0 || currY + i < 0 || currX + j >= w || currY + i >= h)
                        continue;
                    if (binaryImage[(currX + j) + (currY + i) * w] &&
                        !visited[(currX + j) + (currY + i) * w]) {
        ...
        if (!endReached) {
            ...
            if (iter_count == 3) {
                ...
                __android_log_print(ANDROID_LOG_VERBOSE, "beacon",
                                    "igkstra can't fine any suitable path ");
                return;
            }
    ...
    if (drawCostMap) {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                for (int i = 0; i < 3; i++)
                    outputImage[3 * (x + y * w) + i] = static_cast<unsigned char>(255 * multiplier *
                                                                                  pathValue[x +
                                                                                            y * w]);
            }
        }
...
extern "C" {
    ...
    Java_com_example_android_camera2basic_ui_beaconview_CvUtil_processPathPlanning
        ...
        __android_log_print(ANDROID_LOG_VERBOSE, "beacon", "height =%d  width=%d   ", image.rows,
                        image.cols);
        ...
        __android_log_print(ANDROID_LOG_VERBOSE, LOGTAG,
                        "startx =%d  starty=%d    endx =%d  endy=%d   ", startX, startY, endX,
                        endY);
        int w = image.cols;
        int h = image.rows;
        ...
        path_count = 0;
        getPathThroughMaze(outputImage, binaryImage.data, w, h, startX, startY, endX, endY, false,
                       image_out);
